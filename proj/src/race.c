...
void race_init(const char *text, size_t l)
{
  ...
  traffic_light = create_asprite(600, 70, 0, 0, 30, 3, traffic_light_red_xpm, traffic_light_yellow_xpm, traffic_light_green_xpm);
...
void race_process_timer_int(Menu_state *state, uint32_t counter, Sprite* mouse) {
  ...
  if (light && begin_race && no_seconds == 1) {
  ...
  // In case an alarm have been generated by RTC
  if (get_alarm()) {
    use_alarm();
    *state = RESULTS;
    CPM = 0;
